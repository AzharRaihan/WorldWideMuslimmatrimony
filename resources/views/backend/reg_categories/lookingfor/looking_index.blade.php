...
@section('title', 'Lookingfor||Category')
@push('css')
    <!-- DataTables -->
    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card-box">
                        <h4 class="m-t-0 header-title">Looking For Category</h4>
                        <a href="{{ route('admin.looking.create') }}">
                        ...
                        <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            ...
                            <tbody>
                                @foreach ($datas as $key=>$data)
                                    <tr>
                                        <td>{{ $key+1 }}</td>
                                        <td>{{ $data->lookingforname }}</td>
                                        <td>{{ $data->created_at }}</td>
                                        <td>{{ $data->updated_at }}</td>
                                        <td class="d-flex justify-content-center">
                                            <a href="{{ route('admin.looking.edit', $data->id) }}"><i class="fa fa-edit"></i></a>
                                            <a href="{{ route('admin.looking.show', $data->id) }}" class="px-2"><i class="fa fa-eye"></i></a>
                                            <a type="button"
                                            onclick="deletePost({{ $data->id }})">
                                            <i class="fa fa-trash"></i>
                                            </a>
                                            <form id="delete-form-{{ $data->id }}"
                                                action="{{ route('admin.looking.destroy', $data->id) }}" method="POST"
                                                style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
